module CostExplorerHelper::GtlHelper
    ...
    def render_gtl_outer(no_flash_div)
      ...
      logger.debug("render_gtl_outer()=#{options}")
      render_gtl(options)
    end
...
EOJ
    ...
    def gtl_show_url(options)
      # logger.debug("gtl_show_url(#{options})")
      ...
      # url = view_to_url(options[:view], options[:parent]) if options[:view].present? && options[:view].db.present?
      # logger.debug("url = #{url}")
      return "/cost_explorer/show"
    end
  end
